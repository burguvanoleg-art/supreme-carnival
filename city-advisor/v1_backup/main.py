...
async def advisor(query: str = Query(..., description="E.g., 'I want to play billiards tomorrow in Chisinau'")) -> Dict[str, Any]:
    ...
    try:
        ...
        extraction_prompt = (
            ...
            "1. city: City name (return null if not specified)\n"
            "2. date: Date in YYYY-MM-DD\n"
            "3. activity: OSM tag (e.g., 'leisure=billiards')\n"
            "Return ONLY JSON."
        )
        ...
        if not city:
            return {
                ...
                "data": {
                    "missing_city": True,
                    "recommendation": "I'd love to help, but could you please specify which city you're interested in?"
                }
            }
        ...
        final_prompt = (
            ...
            f"User intent: {query}.\n"
            "Task: Suggest where to go, what to do, and what to wear. Concise and friendly.\n"
            "IMPORTANT: Do NOT use any Markdown symbols like **, #, or *. Use plain text only.\n"
            "At the end of your text, add a line: 'FEATURED_VENUE: [Name of a place you recommend]'"
        )
        
        synthesis_res = model.generate_content(final_prompt)
        full_text = synthesis_res.text.replace("**", "").replace("*", "").replace("#", "").strip()
        
        # Extract featured venue
        featured_venue = None
        if "FEATURED_VENUE:" in full_text:
            parts = full_text.split("FEATURED_VENUE:")
            recommendation = parts[0].strip()
            featured_name = parts[1].strip()
            featured_venue = {"name": featured_name, "address": "AI Recommended Suggestion"}
        else:
